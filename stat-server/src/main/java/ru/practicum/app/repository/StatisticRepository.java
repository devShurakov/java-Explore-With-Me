...
public interface StatisticRepository extends JpaRepository<EndpointHit, Integer> {
    ...
    @Query(value = "SELECT count(s.id) as hits, s.app as app, s.uri as uri  FROM stata s " +
            "WHERE " +
    ...
    @Query(value = "SELECT count(DISTINCT s.ip) as hits, s.app as app, s.uri as uri  FROM stata s " +
            "WHERE " +
            " (s.timestamp between :start and :end)" +
            " and (s.uri = :uri) group by s.app, s.uri",
            nativeQuery = true)
